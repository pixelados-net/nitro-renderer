...
export class GraphicAssetCollection implements IGraphicAssetCollection
{
    ...
    private _textures: Map<string, Texture<TextureSource>>;
    ...
    private createAsset(name: string, source: string, texture: Texture<TextureSource>, flipH: boolean, flipV: boolean, x: number, y: number, usesPalette: boolean): boolean
    {
    ...
    private replaceAsset(name: string, source: string, texture: Texture<TextureSource>, flipH: boolean, flipV: boolean, x: number, y: number, usesPalette: boolean): boolean
    {
    ...
    public addAsset(name: string, texture: Texture<TextureSource>, override: boolean, x: number = 0, y: number = 0, flipH: boolean = false, flipV: boolean = false): boolean
    {
        ...
        if(override)
        {
            ...
            existingTexture.orig.copyFrom(texture.orig);

            if(existingTexture.trim && texture.trim) existingTexture.trim.copyFrom(texture.trim);
    ...
    public getLibraryAsset(name: string): Texture<TextureSource>
    {
        if(!name) return null;
    ...
    private addLibraryAsset(textures: Dict<Texture<TextureSource>>): void
    {
        if(!textures) return;
