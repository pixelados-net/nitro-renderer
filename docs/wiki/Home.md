...
2. **[[ARCHITECTURE]]** — the map: source layout, the manager table, the full-client lifecycle at a glance.
...
4. **[[NETWORKING]]** → **[[PACKET-PROTOCOL]]** — how bytes get on and off the wire: the connection lifecycle, then the exact EvaWire binary framing and the composer/parser/event pattern, ending with a step-by-step guide to adding a new packet handler.
...
6. **[[AVATAR-FIGURES]]** → **[[AVATAR-RENDERING]]** → **[[AVATAR-ANIMATIONS]]** — an avatar from the outside in: what a figure string means, how it's composited into pixels, and how postures/gestures/dances/effects make it move (with a full worked animation example).
7. **[[ROOM-ENGINE]]** → **[[ROOM-RENDERING]]** → **[[FURNITURE]]** — a room from the outside in: the isometric coordinate system and tile map, how objects get depth-sorted and painted, and how furniture picks its visual behavior.
8. **[[STANDALONE-RENDERING]]** — everything above, usable without a server connection at all: the `/standalone` entrypoint used for CMS/catalog integrations.
