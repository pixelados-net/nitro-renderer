...
## Layer 2 — `RoomSpriteCanvas.render`: the animation-fps gate
...
- `UPDATE_TIME_INCREASER = 41ms` — a hard wall-clock floor: the method returns immediately if less than 41ms have passed since its last real update, regardless of how often it's invoked.
